...
class ComplyCubeRN{
    ...
    _validateSettings(settings){
        if(!settings)
            return this.onError("Settings are not defined");
        // Validating required settings
        if(!settings.clientID)
            return this.onError("Client ID is not defined");
        if(!settings.clientToken)
            return this.onError("Client Token is not defined");
        if(!settings.stages)
            return this.onError("Stages are not defined");
        // Stages validation
        ...
        const _invalideStages = settings.stages.filter(stage => stages.includes(typeof stage == 'string' ? stage : stage.name) == false)
        ...
        if(_requiredStages.length == 0)
            return this.onError("Required stages are not defined: " + _requiredStages.join(", "));
        // Doucments validation
        const documents = settings.stages.filter(stage => 
            typeof stage === 'object' 
            && stage.name 
            ? stage.name 
        ...
        if(settings.scheme && typeof settings.scheme === 'object'){
            const _invalidSchemes = Object.keys(settings.scheme).filter(key => !schemesColors.includes(key))
            if(_invalidSchemes.length > 0)
                return this.onError("Invalid scheme colors: " + _invalidSchemes.join(", "));
        }
        // Check Color validity
        if(Object.keys(settings.scheme).filter(idx => typeof settings.scheme[idx] == 'string' && settings.scheme[idx].match(/^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$/) == null).length > 0)
    ...
    addHandlers(successHandler = d => {}, errorHandler = e => {}, cancelHandler = e => {}){
        if(!this.emiter)
            return
